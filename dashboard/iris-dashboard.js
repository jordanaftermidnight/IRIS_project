...
A comprehensive response that balances depth with clarity. The ${provider.model} model excels at understanding context and providing nuanced, accurate answers that directly address your specific needs.`;
    ...
    /**
     * Escape HTML entities so model output cannot inject scripts when
     * the result is later assigned to innerHTML. Run BEFORE markdown
     * replacements so the <p>/<br>/<pre>/<code> we add are the only
     * HTML in the output.
     */
    escapeHtml(str) {
        return String(str)
    ...
    formatResponse(content) {
        // Escape model output first — anything else is XSS via innerHTML.
        // Then layer on the markdown structure we want to render.
